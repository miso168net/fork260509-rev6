...
│   ├── view-render-guard.py         管理頁原始 HTML 注入寫法閘：check＝逐行掃 `base-web/src/views/manage/**`（`.vue`／`.ts`／`.tsx`／`.js`／`.jsx`）之被禁字面（模板原始 HTML 指令、DOM 原始 HTML 屬性與插入 API；不分註解與碼）、命中 rc 1 指名檔:行、工作樹或射程缺席與零受掃檔＝rc 2／test＝離線自測；零 docker；pre-commit view-render-guard 段＝base-web pin bump 或本檔 staged 時實跑（base-web 工作樹未就位＝hook 段具名跳過）
