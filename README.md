...
│   ├── comment-overlap.py           rev5↔rev6 註解逐字重疊核：對 rev5 對應檔（同相對路徑；缺席且檔名 `rev6-` 起首者映射同目錄 `rev5-` 同名檔）量共同子字串比（預設 40 字元／5%、超標 rc 1；解析 `//` 行＋`/* */` 塊註解＋`.vue` 之 `<!-- -->`；base-web 路徑三型豁免後量；全路徑碼面豁免〔code span／三反引號圍欄〕；比對面為空 rc 2）／test 自測（RL-0076 量尺；非閘工具＝`NON_GATE_TOOLS` 成員、自測入 pre-commit 條件觸發名冊）
