...
│   └── orchestration/               Workflow 編排骨架 _sk_*.js（單一骨架、TDD／review 兩種主流程共用首段；_sk_rules.js＝generate 產物）、assemble.py 組裝器（三道自檢）、harness-test 十二案／harness-review 九案（斷言＋退出碼）、cdp.mjs、EXAMPLE 成品與 TDD／review 單元定義範本
...
├── .githooks/                       外層 hooks（core.hooksPath）：pre-commit（betterleaks→check＋lint→條件自測→rust-fmt／wire-schema／fork-delta→entity-drift（快照缺席即紅）／schema-frozen 皆條件實跑；雙錨門檻＝pre-commit 檔頭常數）、pre-push（範圍掃描）
│   ├── pre-commit、pre-push
...
- **守門**：pre-commit 一次跑完（秒級）——betterleaks 樣式層 → `docsync check`（GT-01 零漂移）＋`docsync lint`（GT-01～GT-12；GT-01 與 check 同源）→ staged 工具自測 → rust-fmt／wire-schema／fork-delta（rust-api／base-web pin bump、工具本體或憲法 staged 時條件實跑；名冊＝RUNBOOK §12 碼面閘表）→ entity-drift（rust-api pin bump／schema 快照 staged 時條件實跑；快照缺席即紅）→ schema-frozen（凍結 fixtures／data-model／schema-definition staged 時條件實跑）。閘名冊＝`docs/generated/GATES.md`（九欄；Day-1 豁免逐筆帶解除謂詞、到期即紅）。
