...
**Tech Stack:** Markdown；`tools/docsync`（generate／check／lint／test；唯一改動＝`gates.py` 刪一鍵一 helper）；git（`-F` 檔訊息）。

**Spec:** 啟動書 `docs/brainstorms/000-doc-architecture.md`（§0.3 A 六條、§4.3 三指標、§4.6 Day-1 四欄、§5 波 5 列、§7 自評形、附錄 D 未決）；波 4 計畫 `docs/brainstorms/000-w4-rad-ai-tradeoffs.md` §0.3；本檔 §0.1 三題拍板。

---

## 0. 本 brainstorm 的拍板與判斷

### 0.1 user 拍板（2026-09-03；一題一問、首選項為建議）
...
5. **GT-03 豁免解除＝同 commit**：review 事件一入帳，`_has_close_events` 為真、鍵仍登記即 GT-12 ERROR「已到期」；T2 同 commit 自 `gates.py` DAY1_EXEMPTIONS 刪 `GT-03.no-close-events` 與已無人用的 `_has_close_events`（自己的改動造成的孤兒才清）；`events.py` 第 311 行的零事件 SKIP 訊息不動（腿級守衛、此後不會觸發）。tests 88 不變（`test_gates` 逐鍵泛迭代、`test_events` 用合成空事件）。
6. **活書同步一列**：`docs/arc42/11-risks-and-technical-debt.md` ※11.2「Day-1 豁免兩筆」→「一筆（GT-08.lessons-absent）」、去處欄同步；GATES.md 由 generate 重生（Day-1 1 筆）。
...
- 工具：只改 `tools/docsync/gates.py`（刪一鍵一 helper）；`python3 tools/docsync test` 88 綠；lint 0 錯 0 警（跳過 1＝GT-08）；check 零漂移；閘數恆 12。
...
docs/arc42/11-risks-and-technical-debt.md             ※11.2 Day-1 列 兩筆→一筆（T2）
...
範圍＝啟動書 §0.3 A 六條與 §7 addressability 自評；對象＝rev6-admin-root @ <T2 起手 SHA>（波 1～4 已收單）；評估者＝主線 Claude、user 拍板。一次性驗收：分數隨刀變、日後重評另落新日期報告。
...
| Annex IV | `docs/compliance/annex-iv-checklist.md`、`annex-iv-mapping.md` | 不適用（23 鍵逐鍵附理由） | 不適用（法規射程為系統） | — | 檢核表自評欄 23 列「不適用」 |

## 2. DoD A 六條（啟動書 §0.3 A）

| 條 | 機器判／命令 | 結果（執行時實值） | 勾 |
|---|---|---|---|
| A1 對照總表逐列兌現 | `python3 tools/docsync lint` GT-10 零 finding；`docs/generated/RAD-AI-MAP.md` 流程層八列 N/N、系統層十二列「目前無」；非驗收列不入表 | <實值> | ✓ |
| A2 addressability 自評 | 本報告 §1（十二列）＋`grep -c '| 不適用 |' docs/compliance/annex-iv-checklist.md`＝23 | <實值> | ✓ |
| A3 三指標落地 | `docs/generated/STATE.md` 三指標三列＝n/a（零刀）；算式＝`tools/docsync/events.py` `metrics`（資料源／窗口／空值語意寫死） | <實值> | ✓ |
| A4 核心 lint 全綠＋Day-1 逐筆謂詞 | lint 0 錯 0 警（跳過 1＝GT-08）；`docs/generated/GATES.md` 十二列九欄、Day-1 表每列有解除謂詞 | <實值> | ✓ |
...
| 條 | 命令 | 預期 |
|---|---|---|
| A1 | `python3 tools/docsync lint \| grep -c 'GT-10'`；`grep -c '目前無' docs/generated/RAD-AI-MAP.md`；`grep -c '/' docs/generated/RAD-AI-MAP.md` | 0；12；八列 N/N |
...
| A4 | `python3 tools/docsync lint \| tail -1`；`grep -c '^| GT-[0-9][0-9] |' docs/generated/GATES.md`；`grep -c '^| GT-.*\.' docs/generated/GATES.md` | 0 錯 0 警 1 跳過；12；1（GT-08） |
...
## 3. 活書一列＋README 一行＋gates.py（T2 逐字）
...
3. `tools/docsync/gates.py`：刪 DAY1_EXEMPTIONS 的 `"GT-03.no-close-events"` 一行；刪 `def _has_close_events(ctx):` 函式（含其後空行）；若 `ev_mod` import 因此無人用則一併刪。
...
**Files:** Create `docs/reviews/20260903-doc-genesis.md`；Modify `docs/ops/events.jsonl`、`tools/docsync/gates.py`、`docs/arc42/11-risks-and-technical-debt.md`、`README.md`。
...
- [ ] **Step 4: gates.py 移鍵＋活書一列＋README 一行**（本檔 §3 逐字）。
- [ ] **Step 5: 驗**：`python3 tools/docsync test` 88 綠；`python3 tools/docsync generate`（GATES Day-1 1 筆、MILESTONES 加 review 列、STATE events 10）；`git add docs/generated`；`python3 tools/docsync lint` 0 錯 0 警 **1 跳過**（GT-03 腿正式跑：report 存在、wontfix_adr 空）；`python3 tools/docsync check` 零漂移；`grep -c 'no-close-events' tools/docsync/gates.py docs/generated/GATES.md docs/arc42/11-risks-and-technical-debt.md`＝0。
- [ ] **Step 6: Commit**（`-F` 檔）：`docs(review): 文件創世驗收報告＋review 事件；GT-03 Day-1 豁免解除（gates.py 移鍵）；§11.2 Day-1 一筆；README 樹 docs/reviews/`；progress 檔記 T2。
...
- [ ] **Step 1: 出口 checklist**：①`grep -c '^| GT-.*\.' docs/generated/GATES.md`＝1 ②`python3 tools/docsync test` 88 ③lint 0 錯 0 警 1 跳過 ④check 零漂移 ⑤`bash tools/bootstrap.sh` rc 0 且零 ⚠（跑完重新 `cd`）⑥STATE 預算表八列「內」（GT-12 N≥6 進 ERROR 模式前核）⑦`ls tmp/` 只剩 session 檔 ⑧RULES-VERSION `c7a137209e0e`。
...
- **Type consistency**：報告檔名在 §1、檔案結構、T2 Step 2／3、T3 一致；review 事件欄名與 `events.py` schema 一致；Day-1 計數 2→1 在 §0.2-6、§3、T2 Step 5、T3 ① 一致；tests 88 不變。
