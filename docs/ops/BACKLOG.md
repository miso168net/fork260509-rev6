...
- BL-00003｜governance｜閘補腿群（000-r1 R1-001／R1-027／R1-C301）三腿：①GT-03 補 BL 引用存在性腿（review.to_backlog、feature_close／misc 的 backlog_add／done；★backlog_done 指向已刪列，存在性須查事件史曾 backlog_add 或 git 史曾為列、不能只查現行 BACKLOG）②七個自稱 Day-1 的 SKIP 分支登記 DAY1_EXEMPTIONS 或改 ERROR（RL-0051）③CLAUDE.md／憲法內容型主張零閘（R1-C301、部分矩陣見 `docs/reviews/20260904-doc-governance.md`；r1 只做檔×閘分類覆蓋、未做逐條主張×閘×真源矩陣）——①②為工具補腿、逐項一正一反自證、不占閘數；③拍板 2026-09-07：先由 000-r2 一支 lens 產出「CLAUDE.md／憲法逐條主張 × 現有閘覆蓋 × 機器真源」矩陣，有實證清單再決定補既有閘腿（不占閘數）或新閘（一進一出＝ADR-00011，數值續自其翻案之 ADR-00004 表）、現在不立閘｜觸發：①②＝000-r2 修單；③＝000-r2 矩陣落地後拍板
