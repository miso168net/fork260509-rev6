...
- **碼面閘**＝`tools/` 頂層對子庫碼或跨端契約做 check 的系統面機器閘（隨刀進場、不計入 GT-12 治理閘預算、名冊＝RUNBOOK §12 碼面閘表；環境缺席語意逐支見該表「觸發時機」欄——容器依賴且入 pre-commit 者＝具名跳過、工具缺席＝fail-loud（ADR-00019），其餘不類推；ADR-00032）；**治理閘**＝GT-NN（名冊＝GATES.md）。
