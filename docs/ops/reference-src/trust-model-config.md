# trust-model-config.md — 信任模型設定檔（TOML）契約（跨刀活體；RUNBOOK §16 與活書 §3 的權威）

> **本檔＝該契約的跨刀活體家**（ADR-00041）：隨刀前進，現在式面（RUNBOOK／活書／工具）一律引本檔。
> 凍結存證＝`specs/004-ip-trust-anchor/contracts/trust-model-config.md`（該刀收刀當下的定稿、不再前進；兩者自 ADR-00041 之日起分岔，分岔由本檔與 git 史解釋）。
