...
- 首刀＝001 schema 基線刀（波 6 起＝刀、走 SDD 全程）：階段 0 定稿＝`docs/brainstorms/001-schema-baseline.md`（刀序 001→008 承 rev5、D13；各刀範圍由各自 brainstorm 定）；specify、clarify、plan、tasks、analyze 已完成（分支 `001-schema-baseline`；憲法 1.1.0＋ADR-00009 accepted 已落；ADR-00010 proposed、U2 收尾轉 accepted；版本三源拍板：sea-orm 1.1.20、async-trait 0.1.92、rust 1.96.1）；analyze 11 項（HIGH 1＝adapter 假 DSN 改執行期串接、其餘對齊已套用）；TDD 編排進行中（tasks.md 41 條、單元對映 research R11）：Task 0（BL-00001 骨架收斂）、U1（rust-api 骨架＋17 檔承襲＋adapter＋dev stack 重放）、U2（兩閘工具隨遷＋fixtures 雙源互證凍結＋ADR-00010 accepted）已落、LL-00001 首條教訓已落；下一步＝U3（docsync refresh＋兩真表＋entity-drift hook）→ U4（RUNBOOK／活書／DoD 鏈／perf）→ U5（BL-00005、拍板級）。
