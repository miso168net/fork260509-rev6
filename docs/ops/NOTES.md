...
- **進行中＝004 ip-trust-anchor**（分支 `004-ip-trust-anchor`）：階段 0 brainstorm 已定稿（`docs/brainstorms/004-ip-trust-anchor.md`；定稿 `fe6f8ad`＋grill 兩輪修訂 `bfaba0e`——起手前收斂之 BL-00066〔節流判定改每次由資料庫定案、拔除快取負快取〕與 BL-00078〔刪 rules.yml 永不觸發規則〕皆已拍板、隨本刀兌現；原留給 clarify 的十項候選已於第二輪 grill 定案為該檔 §5）；`/speckit-specify` 已產 `specs/004-ip-trust-anchor/spec.md`（`1fef4d7`；零 NEEDS CLARIFICATION、§5 十項定案直接落 FR）。**下一步＝`/speckit-clarify`**（一題一問、不採內建一次三題形；預期以覆蓋掃描為主）→ `/speckit-plan`（Constitution Check 對照 §IV 九題；research 必列 rev5 對應碼清單＋rev6 拍板差異點、三支依賴版本照 brainstorm G7）→ `/speckit-tasks` → `/speckit-analyze`，每步 auto-commit。003 刀收刀後兩支維護批 `maint-backlog-pre-004`、`maint-backlog-71-46-76` 皆已收（逐條見 `docs/generated/MILESTONES.md`）。
