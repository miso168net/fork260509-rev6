...
- **現在＝005 前維護批（七支輕量軌；user 2026-09-21 逐題裁定）**：順序＝`maint-backlog-79`（已收單）→ `-35`（已收單）→ `-42`（已收單）→ `-80-88-94-99`（已收單）→ `-92-97`（已收單）→ `-89-83-85`（已收單）→ `-86-90`；逐支交付查 `docs/generated/MILESTONES.md`。**下一步＝第 7 支（末支）`maint-backlog-86-90`**（`PageRes<T>` 上移 `envelope.rs`＋handler 共用件收攏 `handler/common.rs`）。★第 6 支之 BL-00083 交付為「門鈴計數**量測面去全域累加相依**」——其條文所載「靠 libtest 名稱字典序」之成因已於該支實測證偽，真成因＝同案內常駐 watcher 與 libtest 預設並行（逐項見該支收單事件）。★第 7 支 `maint-backlog-86-90` 之兩條觸發條件皆未到期（`PageRes` 全樹恰一消費者、`operator_from` 恰兩份），user 2026-09-21 裁定**照做**＝提前執行，同批須改寫 `handler/ip_rule.rs` 的 `PageRes` 原地預告註解（否則其解除謂詞當場成假述）並新記一條 BACKLOG 承載「共用件形狀待 005 第三個寫端 handler 進場時重評」。
