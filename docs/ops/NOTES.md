...
- 波 3（本波）：流程層填實（51 子節、每子節「類比張力：」真句）；官方子節內容自 rev5 藍本消化（arc42 28 處）；`docs/generated/reference/rev5-blueprint-map.md`（rev5 活書 `###` 逐項去處、缺一即紅）；C4-L1／L2 修訂與 C4-E 三檔規則與表填實（16 處）；§12 名詞表。出口＝`TODO(波 3)` 歸零、blueprint-map 零缺。
