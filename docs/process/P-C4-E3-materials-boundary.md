...
本檔以 RAD-AI C4-E3 的形制記載三材質即非確定性邊界：Claude 執筆／機器生成／user 拍板；信心＝lint 與 review、降級＝blocked／escalation、測試＝harness（TDD 形十八案、review 形九案）；系統層對應＝`docs/c4/C4-E3-non-determinism-boundary.md`。每個子節首句標明類比張力（哪裡對得上、哪裡是硬套）。
...
| pre-commit | 機器閘 | betterleaks→check＋lint→工具自測→rust-fmt／wire-schema／fork-delta→entity-drift（快照缺席即紅）／schema-frozen／orchestration（皆條件實跑；接線由 docsync tests 機器守） |
