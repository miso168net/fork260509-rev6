...
| pre-commit | 機器閘 | betterleaks→docsync check（GT-01）＋lint（GT-01～GT-12）→staged 工具自測→rust-fmt／wire-schema／fork-delta（pin bump、工具本體或憲法 staged 時條件實跑）→entity-drift（pin bump／快照 staged 時條件實跑、快照缺席即紅）→schema-frozen（凍結面 staged 時條件實跑） |
