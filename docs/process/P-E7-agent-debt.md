...
| 清單從 task 文字建 | 漏「碰得到」的檔、implementer 正確 blocked | rev5:L-042、rev5:L-052 | 派工單 | 派發前 | 對實碼查值域／建構點／釘值測 | RL-0014（agent 面已守；主線面見下列）、RL-0027 | 已守 | 同上 |
| 清單擋自證偽修正 | 改數字後他處假述無法同批修 | rev5:L-032 | 跨檔敘述 | 改數字／集合時 | grep 枚舉、清單外升級 | RL-0011 | 已守（agent 面）／**主線面再犯 ×3**（LL-00006／LL-00007／LL-00011；四形掃描種子＝該物之名／未來式短語／活書枚舉表／舊數量詞字面） | 同上 |
...
類比張力：部分——RAD-AI 的儀表板是統計面；rev6 只有一句總覽＋指針：未守零列（前兩列「骨架多變體」「OPTS 散落」已於 001 刀 Task 0 收斂）；刻意不守一列（模型陳舊）；其餘皆已守。四指標（治理批比、教訓重複率、BACKLOG 淨流量、檢索性）＝`docs/generated/STATE.md`。
