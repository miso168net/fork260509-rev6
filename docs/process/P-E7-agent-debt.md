...
| 骨架多變體 | 骨架收斂為單一 `_sk_head.js`（共用首段）＋`_sk_cycle.js`／`_sk_main.js`（TDD 形）＋`_sk_review.js`（review 形）；變動段（`_vars`／`_allowed`／`_context`／`_prompts`；review 形＝`_vars`／`_plan`／`_context`）逐單元另寫、組裝成品自帶複本屬必然 | rev6 現況 | 編排骨架 | 每次組裝 | 已收斂；成品由 `assemble.py` 組裝器產出、`harness-test.mjs` 十二案與 `harness-review.mjs` 六案守控制流 | 名冊可見（`docs/generated/reference/agents.md`）＋harness 退出碼 | 已守 | 次個編排刀收刀 |
