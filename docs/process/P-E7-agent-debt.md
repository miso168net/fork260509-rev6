...
|---|---|---|---|---|---|---|---|---|
| 清單外改動 | agent 改允許清單外的檔 | rev5:L-075 | 單元產物 | 每單元 | 清單外依 status 升級、限定式項附外改條款 | RL-0022 | 已守 | 首個編排刀收刀 |
...
| 債型 | 徵狀 | 來源 | 影響面 | 觸發 | 處置 | 守門 | 狀態 | 覆審條件 |
|---|---|---|---|---|---|---|---|---|
| review 沿用被駁論據 | 同一 finding 換句重報 | rev6（ADR-00004） | fix 迴圈 | 每輪 | 次輪 prompt 附前輪駁回清單 | RL-0071 | 已守 | 首個編排刀收刀 |
...
| 債型 | 徵狀 | 來源 | 影響面 | 觸發 | 處置 | 守門 | 狀態 | 覆審條件 |
|---|---|---|---|---|---|---|---|---|
| prompt 事實接地錯 | 錯的「事實」被 agent 照做 | rev5:L-081 | 單元產物 | 寫 prompt 時 | 每條附出處、與碼衝突以碼為準 | RL-0028 | 已守 | 首個編排刀收刀 |
...
| 債型 | 徵狀 | 來源 | 影響面 | 觸發 | 處置 | 守門 | 狀態 | 覆審條件 |
|---|---|---|---|---|---|---|---|---|
| 誤報失敗 | script 把做完的工作報成失敗 | rev5:L-011 | 單元結果 | 每單元 | 狀態欄不跨角色複用、確認輪 | RL-0004 | 已守 | 首個編排刀收刀 |
...
| 骨架多變體 | `_sk_head*.js`／`_sk_main*.js` 三份變體並存 | rev6 現況 | 編排骨架 | 每次組裝 | 收斂為單一骨架（與設定債同去處） | 名冊可見（`docs/generated/reference/agents.md`） | 未守 | 首個編排刀開分支（BL-00001） |
...
類比張力：部分——RAD-AI 的儀表板是統計面；rev6 只有一句總覽＋指針：未守兩列（管線債「骨架多變體」、設定債「OPTS 散落」）同一去處＝BL-00001；刻意不守一列（模型陳舊）；其餘皆已守。三指標（治理批比、教訓重複率、BACKLOG 淨流量）＝`docs/generated/STATE.md`。

### 覆審節奏

類比張力：對得上——條件觸發而非定期：覆審條件欄到期（首個編排刀收刀）時全表覆審；收刀 final holistic review（RL-0073）；不定期獨立輪（`docs/reviews/`）；Day-1 豁免到期紅；新坑→LESSONS 一坑一檔、晉升→RULES 一列。
