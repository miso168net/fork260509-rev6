...
閘數 12／上限 12（一進一出；超限只警告不擋＝ADR-00011）。真源兩層：存在＝原始碼 finding 錨形、語意＝docstring GATE 區塊（GT-12 斷言錨形 ⊆ 區塊、三處名冊同源）。Day-1 豁免 0 筆（§4.6；到期即紅）、環境型具名跳過 4 筆（ADR-00019；rc 0、不到期）。
...
| GT-12.submodule-absent | rust-api 之 .git 不存在 | 同 GT-05：門鈴頻道字面的子庫側常數（server/src/ipgate/mod.rs）無 HEAD 樹可讀；外層側照掃 |
