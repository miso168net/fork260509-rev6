...
| `rust-api/` | Cargo workspace：migration（m0001 結構基線＋m0002 seed 基線、承 rev5 逐位元＝憲法 §I.5 例外②）／entity（15 表 sea-orm 投影）／sea-orm-adapter（例外①、vendored casbin adapter）／server（002 刀進場之 HTTP 服務 crate：router→授權→handler→registry 驗證→facade→`Res` 信封；ROUTES 真表＝`docs/generated/reference/routes.md`）；分支來源紀錄檔 `x_fork.branch-origin.md` | 外層 gitlink pin；容器 `rust-api` 由 front-nginx 反代、依賴 postgres／redis／migrate |
