...
| front-nginx → base-web／rust-api | HTTP（容器網路） | 入口 → 前端靜態／後端 API（prod `/api/*` 前綴、憲法 §II #3）；對 rust-api 另帶轉發標頭契約（逐支標頭與注入／移除條件＝§6.1 島 F 情境；欄位語意之權威＝`docs/ops/reference-src/trust-model-config.md`） |
