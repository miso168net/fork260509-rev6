...
2. **Given** 17 檔已拷入並依語意判準重寫註解（四型失效引用必改、通用註解可同文），**When** 跑 lint（GT-05 掃 tools/ 與子庫 pin 樹），**Then** 零裸前代編號
   （rev5 原 m001 之兩處裸 `rev4:m009`／`rev4` 引用、entity 前代引用皆已帶前綴或改寫；★實查更正＝需改註解 9 檔、通用同文零改 6 檔，非「六檔各一處」，見 tasks ✔ U1 與 rust-api c6c7d42 訊息）。
...
- **FR-006**: 承襲 MUST 滿足四條件：①逐位元自證（去註解後 diff 全等；命令形住 ADR-00009 決定節、實跑結果住 commit 訊息與收刀 `feature_close` 事件——ADR accepted 後不可變）
  ②註解依語意判準重寫：四型失效引用（無前綴前代編號、rev5 語境事實、章節號指到 rev6 不存在的節、repo 外權威）必改——帶 `rev5:`／`rev4:` 前綴或改指 rev6 去處；通用註解可與 rev5 同文；判定＝review 逐檔清單（落該單元 commit 訊息、並作 ADR-00010 證據素材；ADR-00009 只住判準釋義、as-built 不回灌）＋GT-05 機器兜底（掃 tools/ 與子庫 pin 樹綠）③防回歸審查逐檔
