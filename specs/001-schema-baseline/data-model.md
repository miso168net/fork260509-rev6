# data-model — 001-schema-baseline（凍結定稿；rev6 承襲 rev5 終態）

> **權威聲明**：本檔＝`rev5:specs/001-schema-baseline/data-model.md`（rev5 凍結 SHA `7eab28a`）整檔承襲改座標（001 grill／clarify 拍板 2026-09-04、
> 文件非碼、憲法 §I.5 不及）：rev6 資料庫基線＝rev5 終態 15 表 169 欄，欄序／欄名／型別之權威＝本檔；§2／§6／§7 表體（閘工具解析面）**位元不動**。
> rev5 對 rev4 的定稿史（§3 rename map、§4 定稿差異、各表「對 rev4 終態的處置」）以史料形保留、一律 `rev4:`／`rev5:` 前綴；rev6 對 rev5 終態**零偏離**。
> 機器對賬面：gate2 欄序左源＝§2 表體；doccheck＝§2 欄五元組＋§6 索引約束 vs 凍結 fixtures；雙源互證＝rev6 pristine 萃取之 fixtures vs rev5 同名檔逐位元全等（contracts/fixtures.md §2）。
> 承襲時戳：rev5 追補拍板（real_ip 全庫 NN、時戳預設統一 `now()`、DB UTC+0；`rev5:` 2026-08-05）已固化於 §2／§4／§6，rev6 照收。
...
> 射程註記：映射僅曾用於 rev5「vs rev4 快照」的血緣核對場景（`rev5:T012`）；rev6 一切管線（fixtures／refresh
> 快照／entity／seed）一律定稿欄名、工具不再內建映射（research R6）。rev4 快照中 `operator_` 僅出現於此 4 欄欄名與兩個索引
...
以上即 rev5 對 rev4 的全部偏離（rev5 血緣核對之授權偏離集：rename map 4 組＋region 新增＋trace_id 改
text＋real_ip NN＋預設統一 now()——`rev5:T006`／`rev5:T012` 逐項 normalize 後全等）；**rev6 對 rev5 終態的偏離＝零支**
（純承襲、無新能力面；雙源互證＝逐位元全等、無 normalize）。
...
1. `operator_` 前綴欄名與 `trace_id varchar(64)` 為 rev5 拍板已推翻之形（rev6 未翻案）——參照 rev4 source
   時**不得帶回**；rev6 承襲物＝rev5 終態、天然不含。
2. seed 簡體原值（role_name／buttons desc）與 password runtime 隨機雜湊為已推翻之形——
   **不得帶回** m0002。
3. `rev4:m003`～`rev4:m015` 為 rev4 後續刀 delta、淨效果已含於 rev5 終態——**不搬**；rev6 第一支 delta 自
   `m0003` 起編（四碼短號＝ADR-00008）。
4. reaper DB role＋GRANTs（`rev4:m012`／`rev4:m013`）**不入基線**——非三節快照射程、屬
   observability 域運維工件、隨該域刀重進場（承 `rev5:R4`、provenance `rev4:ADR 0072`）。
5. fixtures 凍結後永不改寫；基線翻案＝新刀新 ADR（supersedes），不回改本檔。
6. entity 終態版之後刀差異唯一＝`sys_user_role` 兩條真 DB FK Relation（`rev5:002` FR-022）：形狀派生、**保留**；
   其餘 14 檔與 rev5 001 收刀版同（ADR-00010 證據段）。
