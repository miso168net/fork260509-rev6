...
| `src/auth/dev_identity.rs` | — | **D** | 整檔刪 | 汰換射程實查＝`auth/enforce.rs`（`verify` 呼叫點＋cfg 分支收斂）／`auth/mod.rs`／`lib.rs`／`router.rs`（3 處測試 token）／`handler/system_settings.rs`（105 處 dev token 字面之 integration 測試面、斷言不動）／`tests/contract.rs`（14 處）（R7-2） |
...
| `src/handler/system_settings.rs` | — | M | 同 | 105 處 dev token 字面之 integration 測試改真 token 形（斷言不動、FR-033）＋`AppState` 建構點 2 處改五欄＋BL-00041 註解 |
...
| R3-10 | `dev_identity` 汰換須換兩支 lint must-list（`authz_entrypoint_lint.rs`／`entity_access_lint.rs`） | rev6 無 `authz_entrypoint_lint.rs`、`entity_access_lint.rs` 無 dev_identity 指名列 ⇒ 改 `auth/mod.rs`／`lib.rs`／`auth/enforce.rs`（`verify` 收斂）＋三處測試面真 token 化（`router.rs` 3、`handler/system_settings.rs` 105、`tests/contract.rs` 14） | rev6 實碼核實 |
...
2. **`dev_identity` 汰換**：rev6 無 lint must-list（R3-10）；實查引用面＝`auth/enforce.rs`（`crate::auth::dev_identity::authenticate` 真呼叫點＋cfg 分支）、`auth/mod.rs`（宣告＋doc）、`router.rs`（3 處測試 token）、`handler/system_settings.rs`（105 處 dev token 字面、integration 測試面）、`tests/contract.rs`（14 處）；contract 通則 case（rev5 名 `verify_authed_unauthenticated_8888`）改以真 token 形（缺席／非 Bearer／垃圾 token 三形皆 8888、過期 access 得 3333 於 integration 驗）；integration 測試改以 test_kit 簽發真 token（R7-10）。
...
6. **`parse_router_routes` 窄形**：16 條每欄一行、`handler: || get|post(...)` 單動詞（鏈式多動詞靜默通過＝禁）、`method: HttpMethod::X`；generate 後 `docs/generated/reference/routes.md` 恰 16 列（DoD 機器核）。`tools/docsync/tests/test_references.py` 之 `TestRoutes` 為合成語料（實查：`ROUTES_TEXT` 四條合成、無真 repo 釘列），ROUTES 擴表不觸其紅；`router.rs` 自身 `ROUTES_COUNT` 釘值測同批改 16。
...
10. **test_kit 擴充**（rev6 落點＝`server/src/model/facade/test_kit.rs`，承 rev5 `model/mod.rs` 之 `test_db`）：`real_state()`（真 DB＋真 redis＋seed enforcer＋測試 `JwtConfig`＋`captcha_secret`）、`uniq_prefix()`、`SequenceResetGuard`（三支 setval）、`SessionRowsGuard`（DELETE 三表＋清 `sys_user.session_id`）；handler 內 `#[cfg(test)]` 真 DB 案一律掛守衛（RL-0005／RL-0031）。
...
| U1 | 依賴八支進場（R1、features、`cargo update -p log`）＋`config.rs` 六 getter＋`state.rs` 五欄＋錨測＋`AppState` 建構點全改（R7-12：`main.rs`／`router.rs` 2／`handler/system_settings.rs` 2／`tests/common`）＋`main.rs` boot（cache 建連 panic、JwtConfig、captcha_secret、BL-00026）＋兩筆翻案 ADR（AppState／argon2）＋test_kit 擴充（R7-10） | — |
| U2 | `error.rs` 三變體＋`MSG_KEYS` 13（常數形）＋八處測試＋檔頭四處自陳改寫＋`auth/jwt.rs`＋`cache/mod.rs`＋`model/password.rs`＋`request_context.rs`＋`model/mod.rs`／`lib.rs` 模組宣告 | U1 |
| U3 | facade 六支（sys_token／session_event／sys_login_attempt／sys_menu／sys_user／sys_role）＋entity_access_lint 射程 | U1 |
| U4 | `auth/enforce.rs` 真驗章＋denylist 四級降級＋`dev_identity` 汰換（射程＝R7-2 六檔：含 `router.rs`／`handler/system_settings.rs`／`tests/contract.rs` 測試面真 token 化）＋`obs.rs` 三序列 | U2、U3 |
| U5 | `throttle/mod.rs`（三區＋矛盾組合）＋`captcha/mod.rs`＋`handler/captcha.rs`＋`handler/auth/login.rs` 十一步（US4＋US1 後端登入腿） | U4 |
| U6 | `handler/auth/{refresh,logout}.rs`（US2／US3） | U4 |
| U7（獨佔） | `handler/auth/{user_info,alt_stub}.rs`＋`handler/route.rs`＋`handler/mod.rs`＋`router.rs` ROUTES 16（`ROUTES_COUNT`）＋`contract.rs` 16 case＋`wire_schema.rs` 裁判＋generate routes.md 恰 16（`test_references` 合成語料不觸、R7-6） | U5、U6 |
| U8 | base-web：`.env*` 3 檔 4 行＋★軌道九檔（AUTH(a) route store／(b) code-login・register・reset-pwd／(c) captcha.ts／LOGIN-CAPTCHA(i) auth store・pwd-login／LOGOUT-UX(i) user-avatar／I18N(i) request/index.ts）＋新檔 `rev6-auth.d.ts`／`rev6-auth.ts`（七支 wrapper 含 `fetchLoginWithCaptcha`）＋軟區換題接線＋wire-schema 重抽＋`pnpm typecheck`（CONTEXT 收窄：無測試框架） | U0、U7 |
| U9 | i18n：I18N(ii)(iii) 三既有檔（`en-us.ts`／`zh-cn.ts` backend 樹 13 鍵＋`app.d.ts` 型節）＋`zh-tw.ts` 新檔＋`tools/msg-key-gate.py`＋pre-commit 段與 `for` 自測名冊＋test_hook_wiring＋bootstrap 推導＋README＋RUNBOOK §12 列＋跨端閘 ADR＋BL-00037①② | U0、U8 |
| U10 | `tools/walkthrough-baseline.py` 遷入（`BARE_REV5` 復掃）＋`NON_GATE_TOOLS`＋GT-12 自測＋pre-commit `for` 名冊＋RUNBOOK §9c 實文／§12 列與前言現值句／檔頭章節現況句＋BL-00041（兩處註解＋`SUB_SCAN` 子庫腿＋`test_book_ids` 自測） | — |
| U11（收攏） | release profile 首次可跑（R7-13）＋CDP 對照走查（snapshot→三帳號登入／續期／登出／被踢／軟區→清理→diff rc 0）＋schema-gate check＋快速登入鈕 ADR＋活書 §5／§6／§8／§10.2（島 A～E 各一品質情境、刪「目前零情境」句）／§12＋final holistic review＋BACKLOG／NOTES | 全部 |
