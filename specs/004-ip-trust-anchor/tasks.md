...
- [ ] T041 [US3] base-web ★`BASE-WEB-MANAGE-PAGE-WIRING(i)`：`src/locales/langs/en-us.ts` 與 `zh-cn.ts` 各插 `route:` 樹一鍵 `'manage_ip-rule'` 與 `page:` 樹 `manage.ipRule` 子樹（鍵集＝rev5 HEAD `../fork260509-rev5/base-web/src/locales/langs/en-us.ts` 之 `page.manage.ipRule` 逐鍵重打字、本刀零增減）（兩語鍵集相等；各一塊新增型圈界 `[rev6-inline BASE-WEB-MANAGE-PAGE-WIRING(i)+ 004-ip-trust-anchor START/END]`）＋`src/typings/app.d.ts` `Schema.page.manage.ipRule` 型節（一塊新增型圈界）。**DoD：`pnpm typecheck` 在 T042 前紅（`RouteKey` 未含 `manage_ip-rule`）、T042 後綠**
