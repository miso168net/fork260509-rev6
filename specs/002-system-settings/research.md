...
| U2 授權與身分 | enforce／dev_identity／request_context／facade sys_user_role＋未認證 8888 case＋授權矩陣骨架 | rust-api/server/src/auth/**、request_context.rs、model/{mod.rs,facade/mod.rs,facade/sys_user_role.rs,facade/test_kit.rs}、router.rs（只准 build_from 兩 arm 換真 layer＋T024 順修三項）、obs.rs（只准三值補列＋預告句改現在式）、tests/{contract.rs,common/mod.rs（只准補 real_seed_app）,entity_access_lint.rs（只准 is_dir 斷言＋檔頭兩句）} | U1 |
| U3 讀端 US1 | validation registry（含一致性守衛）／facade find_all／handler get／typings 新檔＋service 新檔（契約 §5 兩函式同檔）／wire-schema extract 首抽／wire_schema.rs 裁判／get case／16 鍵全等 integration | rust-api/server/src/{validation.rs,handler/**,model/facade/system_settings.rs,router.rs（只准加一條）}、tests/{wire_schema.rs,contract.rs,common/mod.rs（只准預告措辭改現在式）,fixtures/}、tools/docsync/tests/test_references.py（連動釘值測、只准增列真 repo 釘值——RL-0022）、rust-api/server/src/lib.rs（只准第 6 行預告句改現在式）、base-web/src/{typings/api/rev6-settings.d.ts,service/api/rev6-settings.ts} | U2 |
| U4 寫端 US2 | UpdateReq 三態型／facade update_by_key／handler update／service 寫端走查（零改動）／update case／往返 integration（含同值） | 同 U3 之 rust-api 面（base-web 零改動；service 兩函式已於 U3 落） | U3 |
| U5 驗證失敗 US3 | 紅綠矩陣（型別／範圍／enum／未知鍵／缺席壞形／未知型／不一致）零寫入 integration | validation.rs、handler、tests、error.rs（只准 msg_key 頭註一句改現在式） | U4 |
| U6 越權矩陣 US4 | 三身分×兩端點＋角色軟刪停用案＋ADR ②落 | tests、docs/arc42/decisions、rust-api/server/src/error.rs（只准 5003 變體註解一句改指 ADR ②序號） | U4 |
