...
- [ ] T027 [P] [US1] `rust-api/server/src/validation.rs`：registry 16 鍵 const（data-model §3 原值）＋`is_known_type`＋`declared_type(key)`＋★`check_type_consistency(key, db_type)`（clarify Q1：不一致→Internal 5000）＋`validate(key, value)→canonical`（number：trim→parse i64→界內→to_string；enum：精確成員、大小寫敏感）；TDD 紅綠矩陣（每型合法／非法／未知鍵／未知型／不一致）；＋`validation.rs` U1 佔位頭註改現在式（U1 具名預告）
...
- [ ] T035 [US2] service `rev6-settings.ts` 之 `fetchUpdateSystemSetting(req): request<null>` 已於 T031 同檔落地（U3）——本 task＝寫端走查：`pnpm typecheck` 綠、fork-delta rc 0、`python3 tools/wire-schema.py check` 快照不變（base-web 零改動、`git -C base-web status --porcelain` 空）；rust-api worktree commit＋pin bump（單元邊界）；quickstart §3 首例走查記 commit 訊息
