...
- [ ] T042 [P] [US6] `docs/ops/BACKLOG.md` append 兩條（取檔頭 next 配號）：msg key 跨端閘（後端名冊 ⊆ 前端字典；觸發＝首個接 i18n 的前端刀）；registry 跨鍵不變式（min≤max、captcha_after≤max_fails；觸發＝004 ip-trust-anchor／007 user-password-admin 消費側進場）；BL-00008／00010／00011／00024 於收刀事件 `backlog_done`（簿記 commit 刪列、本 task 只預告）
