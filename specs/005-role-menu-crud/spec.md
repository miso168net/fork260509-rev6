...
> 不可改父；H2 增補同步失敗保留上一份）、§III.2 管理頁 ★ 軌道加用途 (ii)（恰九檔）、島 E 補兩句、兩條憲法簿記句改對。`MSG_KEYS` 19→43（拒因 24 鍵）。**零 migration、零 seed 變更**。
...
### Session 2026-09-24（`/speckit-analyze` 期裁定）

- Q: 超管新增選單時，路由名與前端內建路由同名（如 `login`）要不要擋？ → A: 擋、回 `biz.menu.routeNameExists`（零新鍵）——保留集＝內建常量路由與內建根路由之名集、rust 單一常數由跨子庫閘對賬（ADR-00044 決定 8；憲法 §I.2 builtin 常量集不動；FR-029、US2 場景 3）。
- Q: upstream 同 URL 之 `Api.SystemManage.{RoleList,MenuList}` 與本刀 wire 型逐欄不同，怎麼處理？ → A: 記帳——偏離帳落 ADR-00044 決定 9、欄差集由 wire 裁判面機器導出並釘住（憲法 §I.3 型別謊言帳本；FR-005、FR-071）；wire 行為零變化。
...
3. **Given** 新增，**When** 提交 `parentId=0`，**Then** 豁免父驗證；**When** 父不存在或已刪，**Then** 拒（`biz.menu.parentNotFound`；停用之父不擋）；**When** 路由名與活性列重複，**Then** 拒（`biz.menu.routeNameExists`；
   併發同鍵亦收斂為同一拒因）；**When** 路由名不合形制，**Then** 拒（`biz.menu.routeNameInvalid`）；**When** 路由名為前端保留路由名（內建常量路由或內建根路由之名，例 `login`），**Then** 拒（`biz.menu.routeNameExists`、Clarifications 2026-09-24）；**When** 常量選單欲掛於非常量父之下，**Then** 拒（`biz.menu.constantParent`）。
...
2. **Given** 判定面重建任一步注入失敗，**When** 移除面寫端 commit 後觸發同步，**Then** 不產出新判定面、保留舊面、發結構化告警並計數；有界重試（次數與退避為寫死常數）耗盡仍失敗＝維持舊面、服務不中斷、每次失敗與耗盡各有結構化告警與計數可查（寫端回應不受影響）。
3. **Given** 選單 M 有現役授權，**When** 刪除 M 後以同路由名新增，**Then**（判定面同步成功時）新選單不經任何路徑（現役殘留、判定面殘留、回收桶復原）繼承舊實例授權（資料庫面＋判定面雙斷言）。
...
- 請求 body 缺席或壞形 → 依 FR-009 逐端點收斂（新增→首腿形制拒；部分更新型→零變更成功；以 id 定位者〔含 updateRoleHome〕→id=0→`notFound`；批刪→空陣列 no-op）；授權中介層不讀 body、不因此前移守門。
...
- 批刪清單含查無 id（不存在或已刪）→ 整批拒 `notFound`（Clarifications Q3；兩管理員併發時後送者重新整理再送）；重複 id → 先去重再處理（Clarifications Q4；前端勾選不會送重複 id、只有直打 API 會遇到）；查無與其他守門並存時之先序兩域不同（FR-037）。
- 保留路由名：前端常量路由表以路由名為鍵、後端常量列排在內建之後，與內建路由同名即覆蓋之（例：常量選單 `login` 令全站登入頁失效）⇒ 新增時擋保留名（FR-029）；路由名建後不可變、復原前必曾新增 ⇒ 更新與復原結構性不可達。
...
- **FR-005**: 回應 MUST 逐欄白名單構造（絕不序列化原始資料列）：欄名 camelCase；id 為 number 並守 2^53 上界；狀態類以字串 `'1'`／`'2'`；時間為帶 offset 之 RFC3339；`createdBy`／`updatedBy` 以操作者帳號名批次回填（查無→null）；
  角色列不上軟刪欄（角色無回收桶）；選單列帶導出之 `deleted` 布林、頂層 `parentId`＝0。upstream 未改動之消費者（使用者頁抽屜、菜單權限彈窗）所依之 upstream 型別 MUST 同時相容；upstream 同 URL 之 `Api.SystemManage.{RoleList,MenuList}`（本刀後零消費者）與本刀型之偏離記帳於 ADR-00044 決定 9、欄差集由 wire 裁判面機器釘（FR-071）。
- **FR-006**: 狀態類輸入欄（角色與選單之 `status`、選單之 `iconType`，以及**僅新增路徑**之 `menuType`）MUST 恰二值嚴格解析；值域外（含空字串）＝缺席——新增取預設、更新不動、清單不篩選（null：`status`／`menuType`＝缺席、`iconType`＝清空落 NULL＝ADR-00047 決定 2）；停用護欄以「非啟用」判定；更新路徑之 `menuType` 不經值域解析、出現即拒（FR-018）。
...
- **FR-008**: 每支寫端成功且有業務寫入時 MUST 於同一交易恰落一列操作稽核（批次＝逐標的一列）；提前 no-op 成功（更新全欄缺席、批刪空陣列）零稽核；動作詞彙沿既有小寫封閉集（既有五詞 add／update／delete／restore／unlock、本刀只用前四）、標的表以標的表欄區分、零新動作詞；
  操作稽核落列 MUST 為 IP 規則、角色、選單三寫端一致之要求。操作者取自請求上下文；缺席＝拒寫 `5000`、不以佔位補列；該拒寫事件帶 `refused` 欄、MUST NOT 帶 IP 域降級欄、MUST NOT 增計降級計數、以各域自有之事件目標字面發出；操作者上下文之取得 MUST 先於一切守門與提前 no-op（含批刪空陣列之提前成功與 body 缺席收斂）。寫端之審計欄依憲法 §I.6（`*_by`＝操作者 uid；`deleted_at`／`deleted_by` 與 `updated_at`／`updated_by` 各成對寫；提前 no-op 零寫；逐欄見 data-model §1）。
...
- **FR-013**: updateRole 可編欄＝名稱／描述／備註／首頁路由名／狀態（承 rev5 as-built；首頁路由名同 FR-007 可空文字欄語意、亦可經 updateRoleHome 寫）；守門固定序：操作者上下文（缺席 `5000`）→提前 no-op→名稱非空（`nameRequired`）→鎖列查無（`notFound`）→代碼出現（`codeImmutable`）→停用雙護欄（自身所屬→R_SUPER 恆禁）；請求出現 `roleCode`（值不比對）MUST 拒（`codeImmutable`、非靜默忽略）；停用 MUST 過雙護欄——操作者不得停用自身所屬角色（`cannotDisableSelfRole`、成員身分口徑含停用角色）、
...
- **FR-023**: 同鍵重建零繼承（島 H2）：同路由名重建之新選單 MUST NOT 經任何路徑（現役殘留、判定面殘留、回收桶復原）繼承舊實例授權——現役無殘留（序列化域＋刪除連動歸檔掃盡）＋歸檔不可回灌（reason gate）＋判定面同步；唯一例外＝判定面同步重試耗盡窗（FR-048／FR-049；島 H2 降級窗句），恢復＝下次成功同步或重啟。
...
- **FR-026**: getMenuList/v2 MUST 治理域、樹形、頂層計數分頁；未帶 `size`（含查詢串壞形＝視同沒帶）⇒ 回全部頂層及其全深子樹、回應 `size`＝實得頂層數、`current`＝1（FR-056）；帶 `size` 依 FR-054 通則；同層序 MUST 為（排序值升冪、無值在後，再 id 升冪）、
  頂層分頁依此序切（以「擾動實體回列序前後兩次呼叫逐位相等、且等於獨立排序結果」之案釘住，拔排序必紅）；列帶 `menuMemo`。
...
- **FR-029**: addMenu MUST 支援目錄與選單兩型；守門固定序（承 rev5 as-built、多重違規取先序腿）：父驗證→防環→路由名活性唯一（域內先驗＋活性唯一索引之衝突兜底、同鍵 `routeNameExists`）→常量父鏈→路由名形制（`routeNameInvalid`）；
  本刀增補腿接其後：保留路由名（路由名 ∈ 前端內建常量路由與內建根路由之名集→`routeNameExists`；ADR-00044 決定 8）→名稱非空（`nameRequired`）→href 形制→按鈕碼清單形制；零授權寫入（兩步流第一步）；「隱藏於選單」、多分頁等一般欄照 upstream 可寫。
  updateMenu 守門固定序：操作者上下文（缺席 `5000`）→提前 no-op（FR-007）→名稱非空（`nameRequired`）→鎖列查無（`notFound`）→不可變欄出現（`routeNameImmutable` 先於 `menuTypeImmutable`）→受保護列之停用或改父（`protectedMenu`）→改父之父驗證（`parentNotFound`）與防環（`cycleDetected`）→常量父鏈（含清除常量性之後代腿；`constantParent`）→href 形制→按鈕碼清單形制（先於絕版計算）；逐腿一案並含至少一個多違規取先序腿案；此序凍結入契約。
...
- **FR-032**: updateMenu 之按鈕碼變更（`buttons: null` 或 `[]`＝舊碼全數視為自清單移除）：自清單移除且**絕版**（不再屬任何其他未刪選單〔含停用〕之按鈕碼聯集；判定 MUST 排除標的自身）之碼，其按鈕維政策 MUST 同交易歸檔（reason＝`menu_button_removed`）；
...
- **FR-034**: batchDeleteMenu MUST 子先於父之拓撲序、逐項全套守門、任一違規整批拒、單一交易、至多一次收尾同步；各標的之獨有按鈕碼 MUST 於其歸檔時點現算（同批先行軟刪者不在治理域；同批共持而批外無持有者之碼於批內恰歸檔一次）、MUST NOT 於守門期預算；空陣列＝提前 no-op 成功（零副作用、零稽核、不取選單域鎖）。
...
- **FR-037**: 批次刪除（角色與選單兩域）之查無 id（不存在或已刪）＝屬違規、整批拒、回該域 `notFound`、零變更零稽核（Clarifications Q3）；重複 id＝先去重再處理（例 `[5,5]` 視同 `[5]`；稽核列數＝去重後標的數；Clarifications Q4）；兩者各以 contract 案釘住。
  ★查無與其他守門並存時之先序依各域處理序（承 rev5 as-built；data-model §5.3／§5.7）：角色批刪依 id 升冪逐項（鎖列查無與 seeded／in-use／self-role 同序、先遇先回；例 `[1, <不存在 id>]`→`seededProtected`）；選單批刪先鎖讀全部標的（任一查無即 `notFound`）、再依子先於父逐項守門；兩域各一混合違規 contract 案。
...
- **FR-048**: 同步失敗 MUST 保留上一份已知良好判定面＋結構化告警＋同步結果計數（ok／retry／exhausted 三值，與既有告警規則 `obs016-casbin-reload-anomaly` 之錨字面對齊、開機預註冊）；
  有界重試（總嘗試次數 3＝首次＋至多 2 次重試；第 n 次失敗後線性退避 50ms×n、末次失敗後不退避；每次失敗計 `retry`、三次全敗另計 `exhausted`；寫死常數、絕不取自輸入）；耗盡仍失敗＝維持舊面、服務不中斷——每次失敗與耗盡各一則結構化 error log＋計數、既有告警規則於其 5 分鐘增量窗內觸發，耗盡後殘留窗之處置依 RUNBOOK（恢復＝下次成功同步或重啟）；同步結果不影響寫端回應（commit 成功即回 `0000`）；該告警規則註解之「（島 G1）」改指島 H2＋判定面同步 ADR（rev6 島 G 尚未入憲）。
...
- **FR-050**: 觸發 MUST 恰為五支移除面寫端之「成功且實際歸檔 ≥1 列」：刪除選單、批刪選單、編輯選單之按鈕碼絕版、刪除角色、批刪角色；被拒／無作用／標的不存在／標的零政策列／無按鈕碼變更 MUST NOT 觸發（早退結構性保證）；
  新增選單、復原選單、角色與選單之啟停 MUST NOT 觸發。ADR、data-model §4 與本條之觸發門字面（「成功且實際歸檔 ≥1 列」與五支寫端名）MUST 相同（兩支 wire 契約以各端點之「實際歸檔 ≥1 列＝同步」承載同一門）、不得寫成「成功即觸發」；非觸發情形之列舉得逐處擴寫（語意 MUST 不出五支之外）。
...
- **FR-070**: 測試 MUST 配業務表清理守衛（角色、選單、授權、授權歸檔、指派五表）＋守衛自證測：角色／選單／授權／歸檔四表 arm 時現讀帶界 id 水位與序列值、Drop 先刪水位以上之列再以現讀值還原序列；指派表（複合主鍵、無 id 與序列）arm 時快照鍵集＋角色帶界水位、Drop 刪鍵集差 ∪ `role_id` 逾水位者；seed 態之序列期望只作自證前提；
  指派列之清理先於角色列；角色／選單造列一律顯式大 id 且號段集中登記（指派列之 `role_id` 指向號段角色）（`casbin_rule` 除外：凡需判定面持有之政策列 MUST 經真判定面之新增政策路徑造列、取序列 nextval——如此判定面才真的持有該授權；純資料面案得以號段顯式 id 直插；兩者皆由帶界水位守衛清列並以 arm 時現讀之序列值還原）；測試 MUST NOT 留下對 seed 列之 UPDATE（gate2 seed 逐列比對面）——寫 seed 列之案 MUST 以列快照守衛還原、或改打自建列；稽核斷言取水位窗、不寫絕對計數；契約測試側另立所需庫態守衛。
- **FR-071**: wire 裁判面 MUST：本刀新增之全部 wire 型（回應＋請求＋query；實數以抽取為準）＋共用分頁信封入受審名冊；「序列化鍵集＝快照鍵集」斷言以表驅動涵蓋全部受審讀型（既有七型＋本刀新讀型＋分頁信封；交叉型佔位之一型具名豁免附理由）；upstream `Api.SystemManage.{Role,Menu}` 與本刀 `RoleRecord`／`MenuRecord` 之欄差集 MUST 等於 ADR-00044 決定 9 之偏離帳、漂移即紅。
...
- **FR-074**: 其餘到期承載 MUST 同刀收：`handler/common.rs` 重評三件（兩件共用件之簽章、`tracing::error!` 留呼叫點之切法、測試側狀態快照家族；結論落檔 doc）；泛型三態、空字串轉 NULL（作用於三態內層＝清空、非缺席）、兩向狀態映射首次出現即落 `handler/common.rs`，系統設定 handler 之私有三態改引（先依 RL-0070 查掃描閘）或於 doc 註明保留理由；角色／選單寫端之拒寫事件 MUST 同時納入既有兩道字面守——各域事件目標字面名冊與「生產碼零降級欄」掃描射程皆擴及角色與選單兩 handler（前代 HEAD 帶降級欄之寫法不帶入）；7777 腿 denylist 鍵改 RAII；請求上下文缺席之每請求總數案（至少打一支角色或選單寫端、釘住「其餘端點 2 格」）；
  dev 信任模型三鏡像「刪一錨一」（對賬三態：環境變數缺席＝具名跳過、在而讀檔失敗＝紅、讀得到＝逐欄比對）；共用分頁信封與其 typings 權威之錨。

**L. 治理、文件與簿記**

- **FR-075**: 憲法 Amendment MUST 一次 MINOR（1.4.0→1.5.0）、獨立 commit（內容恰為憲法、ADR①、README 憲法版本鏡像、活書 08 §8.4 引文改字與 generate 產物——引文隨憲法字面同顆改、否則即成假引文；FR-076 之 ADR②③⑥ 另起施工前提顆）：①§I.7 島 H 五條入憲（以前代 v1.7.0 字面為底；H1 終態句以描述形＋現在式條件句寫成、不帶刀集外刀名；H3 增補受保護選單不可停用、不可改父；H2 增補同步失敗保留上一份之方向句〔單一行程部署下耗盡窗為唯一已知降級窗〕；H3 常量父鏈寫端列舉、H4 兩域消費面、H5 常量標的父鏈重驗之明文化；
  常數留活書；承襲指針表 H 列尾註；MAJOR 射程句「六島」改「七島」；依承襲指針表尾句完成之跨島重審結論載 ADR）②島 E 補兩句（解鎖端點之操作稽核先於解鎖標記寫入；操作者上下文缺席即拒寫 `5000`、不以佔位補列）
  ③§III.2 ★ 軌道加用途 (ii)「role／menu 管理頁 CRUD 接真」恰九檔（role 三檔、menu 兩檔、共用表頭元件、兩語 locale、`app.d.ts`；兩顆授權彈窗與 `menu/modules/shared.ts` 明文不入；範圍欄對三支僅新增型檔寫 rev5 as-built 預估塊數、六支 view／元件檔不預估，實數以標記為準；前端單元出口對三支僅新增型檔逐檔斷言新增型塊數等於預估，不等即停手升級主線、由 user 定當刀 PATCH 或延至下次 Amendment 實數化〔④ 之例外路徑；延後即同顆登記回填義務〕）
  ④表外宣告 1 之量法句與既有列範圍欄數字改為與活書 08 同一量法（BL-00118），並補「範圍欄明文不預估或以預估值填列之列，實數得延至下一次 Amendment 實數化」之例外路徑⑤§I.7 段首增補計數句改為以 Amendment log 為準（BL-00119）。Amendment accepted 前 base-web 既有檔 MUST 零 diff。
...
- **選單序列化域**（概念實體）：單一 advisory 鎖承載之互斥執行域；成員＝選單五寫端＋角色刪除家族（＋授權治理之選單維／按鈕維寫端及授權回收桶復原之該兩維分支；端點維不入域）；交易首動作、固定鎖序。
...
- **SC-003**: 角色守門全可驗——三層守門固定序逐腿一案（in-use 與 self-role 以直種指派構造）；停用雙護欄兩案；代碼形制、活性唯一（含併發同碼一案）、不可變各一案；批刪含違規整批零變更一案、含查無 id 整批拒一案、含重複 id 去重後成功一案、查無與 seeded 並存之先序一案（FR-037）、空陣列 no-op 一案。
- **SC-004**: 選單守門全可驗——父驗證三處、防環（含上溯逾限）、不可變兩欄、常量父鏈（新增／改父／清除常量性之後代腿／復原第四腿）、受保護三腿（刪除／停用／改父；停用與改父各一正一反）、保留路由名一正一反、href 形制、按鈕碼清單形制（非 null 非陣列、成員非物件、缺碼、碼為空或非字串、超長、重複各一）、同層排序擾動案、addMenu／updateMenu／deleteMenu／restoreMenu 四支固定守門序各至少一個多違規取先序腿案、
  子先於父批刪（含查無 id 整批拒、重複 id 去重、查無優先之先序、同批共持按鈕碼恰歸檔一次各一）各至少一案；治理域誤用顯示域之負向測試在案。
...
- **Amendment 與 ADR 親決時點**：plan 期全數產草稿；Amendment 於 tasks T002（Amendment 顆）、施工前提三支於 T003（施工前提顆）親決——同一親決輪、ADR-00043／ADR-00044 之決定節先於 ADR-00042 呈定（島 H 條文引其為凍結位）；已知態兩支於治理單元親決（004 T064 形）。
