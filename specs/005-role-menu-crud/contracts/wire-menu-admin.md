...
語意：空陣列＝提前成功（零副作用、不取域鎖）；重複 id 先去重；鎖讀全部標的、任一查無或已刪＝整批 `notFound`；子先於父（深度 DESC、id DESC）逐項全套 §6 守門（「未刪子項」排除同批已刪者）；任一違規整批拒、單一交易、零變更零稽核；整批合計實際歸檔 ≥1 列＝至多一次同步。
200：`data: null`。
