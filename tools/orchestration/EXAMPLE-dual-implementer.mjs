...
const CONTEXT = [
  ...
  '   · m001：檔頭 `//! m001 — 001-schema-baseline 結構基線…`（改 rev6 語境：`m0001`、承 `rev5:m001` 逐位元、憲法 §I.5 例外②／ADR-00009）；檔頭 §4 授權偏離／§10 防回歸句（改史料語意）；`承 rev4:m009 淨效果`（已前綴、可留）；索引段「idx_*_operator_time 名照 rev4 原樣保留」（語意改「承 rev5 定稿、名稱不動」）。',
  '   · m002：檔頭 `transcribe-m002.py（T008②）`／`seed-decision.json`／`FR-005`／`clarify Q1／Q2`／`research R3`＝rev5 施工史→改寫（帶 `rev5:` 前綴；rev6 seed 內容權威＝本檔本體、凍結證據＝`specs/001-schema-baseline/fixtures/seed.sql`、data-model §8）；sha256 字面屬 rev5 轉錄證據、以 `rev5:` 史料形保留或刪除皆可。',
...
const IMPL_STAGES = [
  {
    ...
    prompt: function (reports) {
      return [
        ...
        '【四】migration（T007／T014／T015）：`cp` rev5 `m001_baseline_schema.rs`→`m0001_baseline_schema.rs`、`m002_baseline_seeds.rs`→`m0002_baseline_seeds.rs`；逐檔只改註解（F 之 m001／m002 條）；自寫 `lib.rs`／`main.rs`（G）。',
