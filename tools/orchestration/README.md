...
- model 字串：`opus[1m]`＝別名、隨預設 Opus 版本前進——現解析為 `claude-opus-5-5[1m]`（探針 run 2026-09-23：persisted `workflows/wf_*.json` 之 `model` 欄；000-r1 當時為 `claude-opus-5[1m]`），骨架常數寫別名、不釘版本；`fable[1m]`＝`claude-fable-5-1`（本 repo 探針 run 2026-09-04：`fable[1m]`／`claude-fable-5-1[1m]`／`claude-fable-5-1` 三種寫法於 persisted `workflows/wf_*.json` 之 `model` 欄皆解析為同一 id——fable 的 1M 為原生、後綴被正規化）。
