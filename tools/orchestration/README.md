...
| 檔 | 內容 |
|---|---|
| `_sk_head.js` | 兩種主流程共用的首段：防呆①args 斷言／②guard（長度、`zh-TW`、冒煙 token、規則塊版本串 `RULES-VERSION`）／③保險絲推導＋自我斷言（TDD 形由 `IMPLEMENTERS` 推、review 形由 `_plan` 段常數推且每 run ≤24 支＝wf-watchdog 25 支底線）／④兩套 schema（WORK＝status，REVIEW＝agentStatus）；**模型家單一真源**＝TDD 三角色 `IMPL_OPTS`／`REVIEW_OPTS`／`FIX_OPTS`＋review 四角色 `LENS_OPTS`／`MIRROR_OPTS`／`GRADER_OPTS`／`CRITIC_OPTS`＋`DEEP_THINK`（生成表＝`docs/generated/reference/agents.md`）。★模式偵測＝`_vars` 段定義 `IMPLEMENTERS`（TDD）或 `REVIEW_STAGE`（review），兩者互斥、皆缺即 throw |
...
| `harness-review.mjs` | review 形 **六案＋逐項斷言＋退出碼**：`node harness-review.mjs <script.mjs>`；期望值自 `REVIEW_STAGE`／`INLINE_VERIFY`／`CRITIC`／`SMOKE` 行與回傳結構推導；樁只替換 `agent()`／`parallel()`／`pipeline()`；秒級 |
...
組裝成品範例＝`EXAMPLE-dual-implementer.mjs`（001 刀 U1 原樣；前代 rev5:008 兩支成品已刪＝000-r1 R1-059／060／062／087／088：內含 rev5 事實與行號形引用）；review 形以兩支 `EXAMPLE-review-*-unitdef.py` 為起手範本、成品由組裝器現產（gitignored `tmp/`）。

## 組裝法

單元定義＝一支 python 模組（`tmp/<刀>-uN.py` 形；範本見兩支 `EXAMPLE-review-*-unitdef.py`、TDD 形見 002 刀各 `tmp/002-u*.py`），只寫變動段的 JS 原文字串；共同段由 `_sk_*.js` 供。
...
**review 形**（`MODE = 'review'`）：`VARS`（meta＋`UNIT`／`FEATURE`／`SMOKE`／`START_LOG`／`REVIEW_STAGE`／`FINDING_CATEGORIES`／explore 另 `INLINE_VERIFY`）／
...
**`agentStatus=failed` 不殺 run**（`failed` 留帳、該 lens 不派兩鏡）／回傳形齊全（`status`／`stage`／`smoke`／`nulls`／`failed`／`summary`／`agentsSpawned`）。
