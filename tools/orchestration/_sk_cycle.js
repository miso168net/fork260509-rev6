...
function fixPrompt(blockers, roundNo) {
  ...
  return [
    ...
    '· 修改一律限在允許清單內。清單外需要動＝**絕不擅改**，依 status 分值升級（`done_with_escalation`＋`escalations` 逐條附修法；★零改動升級不會終止 run——該批 finding 記為已升級主線、審查段照常進行，RL-0025）。',
...
async function cycle(phaseName, reviewPrompt, tag) {
  ...
  for (let r = 0; r <= MAX_FIX_ROUNDS; r++) {
    ...
    if (fx.status === 'done_with_escalation' && changed === 0) {
      ...
      blockers.forEach(function (b) { if (!rejKeys.has(keyOf(b))) { escalated.push(b); escalatedKeys.add(keyOf(b)) } })
      if (rejNow.length === 0) {
