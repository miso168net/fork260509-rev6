...
// 案1：每輪不同 blocker、確認輪清空 → 應收斂，quality 段用滿 4review+3fix=7 支
let n1 = 0
await run('案1 fix 迴圈跑滿→確認輪清空→判收斂（rev5:L-011 變形②：不得把已修好報成 unresolved）', (label) => {
  if (label.includes('implementer')) return OKW
  if (label.startsWith('quality:review')) { n1++; return n1 <= 3 ? B('缺陷' + n1) : CLEAN }
  if (label.startsWith('quality:fix')) return OKW
  return CLEAN
})
...
await run('案2 連兩輪同 blocker（file×summary 相同）→ 判不收斂', (label) => {
  if (label.includes('implementer')) return OKW
  if (label.startsWith('quality:review')) { n2++; return B('同一句摘要') }
  if (label.startsWith('quality:fix')) return OKW
  return CLEAN
})
...
await run('案3 fix 連兩輪零改動 → 判不收斂', (label) => {
  if (label.includes('implementer')) return OKW
  if (label.startsWith('quality:review')) { n3++; return B('缺陷' + n3) }
  if (label.startsWith('quality:fix')) return { status: 'ok', report: 'r', filesChanged: [] }
  return CLEAN
})
...
await run('案4 review agentStatus=failed → 立即 return', (label) => {
  if (label.includes('implementer')) return OKW
  if (label.startsWith('quality:review')) return { agentStatus: 'failed', blockers: [], notes: '工具壞了' }
  return CLEAN
})
...
await run('案5 review 有 blocker（agentStatus=ok）→ fix 必須跑（變形①）', (label) => {
  if (label.includes('implementer')) return OKW
  if (label.startsWith('quality:review')) return B('固定缺陷')
  if (label.startsWith('quality:fix')) return OKW
  return CLEAN
})
...
await run('案8 fix agent blocked → 立即 return', (label) => {
  if (label.includes('implementer')) return OKW
  if (label.startsWith('quality:review')) { n8++; return B('缺陷' + n8) }
  if (label.startsWith('quality:fix')) return { status: 'blocked', report: '要動清單外檔', filesChanged: [], escalations: ['x'] }
  return CLEAN
})

// 案9：保險絲——review 每輪不同 blocker、兩段都跑滿且確認輪仍有 blocker
let n9 = 0
await run('案9 兩段皆跑滿（確認輪仍有 blocker）→ 應在 quality 段就 return、不逾保險絲', (label) => {
  if (label.includes('implementer')) return OKW
...
await run('案10 fix done_with_escalation＋零改動 → 當場 return 升級（rev5:L-078，非不收斂）', (label) => {
  if (label.includes('implementer')) return OKW
  if (label.startsWith('quality:review')) { n10++; return B('清單外檔的缺陷' + n10) }
  if (label.startsWith('quality:fix')) return { status: 'done_with_escalation', report: '該檔在不得動清單', filesChanged: [], escalations: ['data-model.md 那列要主線改'] }
  return CLEAN
})
