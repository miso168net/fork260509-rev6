...
class TestRoutes(unittest.TestCase):
    ...
    HttpMethod／未知 Protection／缺欄／頂層多餘行／來源缺席→各 raise（反）；真 repo 釘現行列集（health／metrics／get-system-settings）。"""
    ...
    # 003 刀 U5 五列＝auth 兩條／route 三條＋U6 一列＝/auth/refreshToken＋U7 一列＝/auth/logout＋U8 一列＝/auth/loginCaptcha＋U9 四列＝替代登入 stub；現行十六列＝data-model §12 終態）。★釘值形刻意保留——逐列全等是各單元的
    ...
    def test_real_repo_pinned_rows(self):
        ...
        self.assertEqual(rows, [("/health", "GET", "Public", "health", True), ("/metrics", "GET", "Public", "metrics", True),
                                ...
                                ("/auth/login", "POST", "Public", "auth-login", False),
                                ("/auth/refreshToken", "POST", "Public", "auth-refresh-token", False),
                                ("/auth/logout", "POST", "Public", "auth-logout", False),
                                ("/auth/getUserInfo", "GET", "Authed", "auth-get-user-info", False),
                                ("/auth/loginCaptcha", "GET", "Public", "auth-login-captcha", False),
                                ("/auth/sendCaptcha", "POST", "Public", "auth-send-captcha", False),
                                ("/auth/codeLogin", "POST", "Public", "auth-code-login", False),
                                ("/auth/register", "POST", "Public", "auth-register", False),
                                ("/auth/resetPwd", "POST", "Public", "auth-reset-pwd", False),
                                ("/route/getConstantRoutes", "GET", "Public", "route-get-constant-routes", False),
                                ("/route/getUserRoutes", "GET", "Authed", "route-get-user-routes", False),
                                ("/route/isRouteExist", "GET", "Authed", "route-is-route-exist", False)])
